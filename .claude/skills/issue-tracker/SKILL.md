---
name: issue-tracker
description: 登记与处理 WorkBuddy 的问题（issue）台账（docs/issues/）。当用户要「登记/记录一个问题」「新建 issue」，或「处理/修复某个 issue」「看还有哪些 issue」时使用。定义了从发现→登记→处理→验证→关闭的完整流程。
---

# WorkBuddy Issue Tracker

WorkBuddy 的问题台账在 `docs/issues/`。核心规矩：**所有问题先登记成一条 issue，再处理**。
...
2. **读上下文**：先读 `CLAUDE.md` 与该 issue 的「建议修法」；遵守项目铁律（见下）。
...
6. **关闭**：`status: fixed`，README 行改 ✅；在 issue 文件末尾追加一小节：
   ...
   - commit：<哈希，如已提交>
   ```
7. **提交**（若用户要求提交）：一条 commit 对应一个（或一组同源）issue，标题带 `WB-###`。提交前跑仓库既有的敏感文件自检：
   `git diff --cached --name-only | grep -iE "\.env$|node_modules|\.venv|\.db|/workspace/|\.png$|\.playwright"` 应为空。
...
- **视觉零重设计**：CSS class 名与设计 token 沿用原型（`docs/workbuddy-v2.html` / `src/styles/`）；暗色是 `body.dark` 上的变量覆盖，别写死会在暗色翻车的浅色背景。
