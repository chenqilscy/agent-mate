"""真·知识库 + 文档（WB-171）—— 项目级团队知识库的 Hub 权威源。

Manager 只**管理**知识库及其文档（元数据 + 文档字节落盘），**绝不算向量**：文档 `vector_status`
恒从 0（未向量化）起，Hub 永不置 1——向量化是执行面（App/本地 backend）将来的事：**只调 GLM 的
...
async def upload_document(
    ...
    Hub 只落盘字节 + 记元数据（vector_status=0），绝不算向量。"""
    _require_write(project_id, account)
    _kb_or_404(project_id, kb_id)
