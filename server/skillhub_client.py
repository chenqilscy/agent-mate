"""SkillHub CLI 客户端（WB-069）—— Server 侧复用本机 skillhub CLI 抓目录/查询。

不新起 HTTP 客户端：跑 `~/.skillhub/skills_store_cli.py`（Server 自己的 Python）。移植自
`backend/agent/skills_store.py` 的硬化封装：白名单转发 env（**绝不透传密钥**，铁律#4）、
强制 `PYTHONUTF8` 避 GBK 崩、失败降级。**安装动作不在 Server 做**（永远留本地）。
...
def _stored_key() -> str:
    """SkillHub API key（WB-095）：优先 Server 库里的平台设置，env 兜底。skh_ 个人 / sk-ent- 企业。"""
    try:
...
def _http_json(url: str, key: str = "", timeout: int = 15) -> Any:
    headers = {"User-Agent": "agentmate-server/0.1", "Accept": "application/json"}
    if key:
...
# 按榜单类型的实时目录代理（WB-186）：App 原先绕过 Console 直连 skillhub.cn（本地 CLI），
# 与 search/preview 遵循的 WB-130「App 不直连 SkillHub、统一经 Console」自相矛盾。
# 这里补齐 Console 侧的对应能力。顺带：Server 走 HTTP 无需 CLI，故没装 CLI 的 App 也能拿到真实榜单。
...
# ── 单技能预览代理（WB-130）───────────────────────────────────────────────
# Console 统一对 SkillHub 取数：App 不再直连 SkillHub，改调本代理。
...
def _cli_skill_md(slug: str) -> tuple[str, str, dict[str, str], list[str]]:
    ...
    if not cli_available():
        return "", "", {}, []
    tmp = Path(tempfile.mkdtemp(prefix="skhub-server-prev-"))
    try:
