...
4. **密钥只存后端，绝不进前端 / 不提交**。LLM API Key 仍**只存** `backend/.env`（`LLM_API_KEY`/`LLM_API_BASE`/`LLM_MODEL`）、不透传给子进程环境（见 WB-011）。连接器 / 机器人 token（如 Telegram bot token）可存后端 **DB 或 `.env`**（DB 已 `.gitignore`、永不提交）——写入者 write-only、**任何响应绝不把 token 值回传前端**（见 WB-077）。
