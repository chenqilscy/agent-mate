// 连接 Hub 弹窗（WB-067 Slice 2）：登录/注册到中心服务，之后以 Hub 账号身份协作。
// 已连接则展示账号 + 导入本地项目到 Hub + 团队通知 + 断开。
...
export function HubConnectModal({ onClose }: { onClose: () => void }) {
  const { linked, connect, disconnect } = useHubStore()
  ...
  useEffect(() => {
    if (!linked) return
    api.hubNotifications().then((r) => setNotifs(r.notifications || [])).catch(() => {})
  }, [linked])
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast('已连接 Hub · ' + name.trim())
    } catch {
      toast(mode === 'login' ? '连接失败：用户名或密码错误，或 Hub 不可达' : '注册失败：用户名可能已被占用，或密码太短（≥4 位）')
      setBusy(false)
    }
  }
  ...
  const doImport = async () => {
    try {
      const r = await api.hubImport()
      toast(`已导入 ${r.imported} 个本地项目到 Hub（跳过 ${r.skipped}）`)
    } catch { toast('导入失败') }
  }

  const markRead = async () => {
    try { await api.hubMarkNotifs(); setNotifs((ns) => ns.map((n) => ({ ...n, read: 1 }))) } catch { /* ignore */ }
  }

  return (
    <div className="np-overlay open" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div className="np-modal auth-modal" role="dialog" aria-modal="true" aria-label="连接 Hub">
        <div className="np-h">连接 Hub<button className="np-x" onClick={onClose}>×</button></div>

        {linked ? (
          <>
            <div className="np-body">
              <div className="np-lbl">已连接为 {linked.name}</div>
              <div className="auth-switch">项目 / 成员 / 评论 / 在线状态经中心 Hub 协作。</div>
            ...
            <div className="np-foot">
              ...
              <button className="btn-ghost danger-b" onClick={() => { disconnect(); toast('已断开 Hub'); onClose() }}>断开</button>
            </div>
          </>
        ) : (
          <>
            <div className="np-body">
              ...
              <div className="np-lbl">Hub 用户名</div>
              <input className="np-input" value={name} autoFocus placeholder="你的 Hub 用户名" onChange={(e) => setName(e.target.value)} onKeyDown={onKey} />
              ...
              <div className="auth-switch">连接后以 Hub 账号身份协作；不连接则一切照常本地运行。</div>
            </div>
            <div className="np-foot">
