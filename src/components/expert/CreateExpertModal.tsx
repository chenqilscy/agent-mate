...
export function CreateExpertModal({ open, onClose, onCreated }: {
  ...
  return (
    <div className="np-overlay open" style={{ zIndex: 160 }} onMouseDown={(e) => { if (e.target === e.currentTarget) close() }}>
      <div className="np-modal" style={{ width: 480 }} role="dialog" aria-modal="true" aria-label="创建专家">
        <div className="np-h">创建专家<button className="np-x" onClick={close}>×</button></div>
        <div className="np-body">
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <input className="np-input" style={{ flex: 1 }} placeholder="专家名称，如「运营增长顾问」" value={name} onChange={(e) => setName(e.target.value)} autoFocus />
          </div>

          <div className="np-lbl">职称 / 一句话身份</div>
          <input className="np-input" placeholder="如：十年增长操盘手" value={subtitle} onChange={(e) => setSubtitle(e.target.value)} />

          <div className="np-lbl">能力介绍</div>
          <textarea className="np-ta" placeholder="这个专家擅长什么、能帮用户解决什么问题（展示用）" value={intro} onChange={(e) => setIntro(e.target.value)} />

          <div className="np-lbl">人格指令<small style={{ color: 'var(--text-3)', fontWeight: 400, marginLeft: 6 }}>召唤时注入，决定它怎么回答</small></div>
          <textarea className="np-ta" placeholder="以某专家身份作答的指令。如：以资深增长操盘手身份作答，先定位漏斗卡点，再给可执行动作，结论先行。留空则用上面的能力介绍。" value={persona} onChange={(e) => setPersona(e.target.value)} />

          <div className="np-lbl">标签<small style={{ color: 'var(--text-3)', fontWeight: 400, marginLeft: 6 }}>逗号分隔</small></div>
          <input className="np-input" placeholder="增长, 漏斗诊断, 投放" value={tags} onChange={(e) => setTags(e.target.value)} />
        </div>
        <div className="np-foot">
          <div style={{ flex: 1 }} />
          <button className="btn-ghost" onClick={close}>取消</button>
          <button className="btn-dark" disabled={!name.trim() || busy} onClick={confirm}>创建</button>
        </div>
      </div>
    </div>
  )
