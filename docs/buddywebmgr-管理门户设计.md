...
- **凭据绝不进 Hub**：LLM key、连接器 token 只在本机 `backend/.env`。连接器 launch spec 里只存**环境变量名**
...
- **缺**：`work_items`（计划/任务）在 Hub 无模型、无同步（App 本地独有）。

**Hub 控制台（`hub/web/console.html`）**：auth / 项目(成员·邀请·讨论·在线·时间线) / 组织 / 通知 / 目录 Admin(裸 JSON)。

**改动集中在门户 UI + 少量 Hub 后端（work_items + 目录写端点细化）；项目配置字段后端已支持 `PATCH /projects`。**

## 4. 目标形态

门户更名 **BuddyWebMgr**，导航重构为：
...
### 5.3 SkillHub（WB-084）
- **浏览**：Hub 镜像的技能目录（`SKILLHUB_*` + `replace_skillhub_mirror` 存的行），分类过滤。
...
App 的 work_items 目前**本地独有**。要在门户管理团队计划/任务，需要 Hub 侧新增并双向同步：
- **Hub 新模型** `work_items`(project_id / title / status[todo·doing·paused·done] / source / assignee / order / created_at)。
- **路由**：`GET/POST/PATCH/DELETE /projects/{id}/work-items`（access-gated，Viewer 只读）。
- **同步**：扩展 WB-062 —— 下行 pull 把 Hub work_items 镜像进本地；上行把本地新增/改状态回传（沿用 outbox 或直连）。冲突以 Hub 为准（团队共享）。
- **门户 UI**：看板（4 列拖拽）+ 列表，与 App 的「计划/任务」tab 对齐。
> 体量最大、且触碰本地⇄Hub 同步。建议放 epic 末尾；若想先要轻量收益可后置或拆二期。
...
| **`work_items`（新）** | Hub 新表 + DAO + 路由 + 本地⇄Hub 同步（WB-081） |
...
|---|---|---|---|
| **WB-078** | epic | 本设计 + 总纲 | — |
| **WB-079** | frontend | 品牌更名 BuddyWebMgr + 导航重构（骨架） | 小 |
...
| **WB-081** | backend | **团队计划/任务** —— Hub `work_items` 模型 + 路由 + 本地⇄Hub 同步 + 门户看板 | 大 |
