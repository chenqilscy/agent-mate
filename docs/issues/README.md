...
| [WB-112](WB-112-manager-positioning-data-spec.md) | 🟡 | P1 | fullstack | AgentMate Console 管理端定位（epic）—— 改名 BuddyWebMgr→AgentMate Console(done) + 数据分层规范 `docs/agentmate-数据分层与同步规范.md`(done) + 统一用户/协作写代理/身份强映射/动态回读/镜像增量合并/PM 细化(待做) |
...
| [WB-191](WB-191-skillhub-mirror-no-delisting.md) | ⬜ | P3 | fullstack | SkillHub 段是上游 skillhub.cn 商店的镜像(369 条)，本地目录下架对它无效 —— 想下架某条需 Console 侧跨同步存活的过滤(replace_all_downlink 每次清空重建，删镜像行必被覆盖)；WB-190 实测发现 |
