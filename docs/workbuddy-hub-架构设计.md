# WorkBuddy Hub —— 架构设计（local-first 执行 + 云端控制平面）

> 状态：设计稿 v1 · 2026-07-07 · 对应 epic [WB-058](issues/WB-058-hub-control-plane-epic.md)
> 关联：[实现方案](workbuddy-实现方案.md) · [CLAUDE.md](../CLAUDE.md)
...
WorkBuddy 现在是纯 **local-first**：后端跑在用户本机 `localhost:8000`，浏览器只是显示器。
...
> 已安装技能（磁盘 `~/.workbuddy/skills/` + SkillHub CLI 真安装/搜索）、项目/会话/消息/待办/自动化/用户/成员/通知（SQLite）。

**结论方向**：立一个独立的中心服务 **WorkBuddy Hub**（控制平面），掌管账号/组织/项目/成员/目录；
