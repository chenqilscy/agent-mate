...
- 甘特、负载、WIP、模板等已经存在的能力维持可用，不继续优先扩充高级 PM；
...
截至 2026-07-21：R0、R1、R2、R3、R5 已满足退出条件。R1 已具备办公产物工具与 10 个黄金任务
身份门禁，R5 已具备独立 Run 的多专家 DAG、审稿/预算/失败边界，并通过两个真实模型对照场景。
R4 的 Skill 不可变版本、capability、灰度、tombstone、回滚以及桌面更新代码链路已经落地；完整退出仍由
WB-257 的真实签名安装/回滚演练阻塞，该演练需要部署侧 HTTPS 产物、Tauri 签名私钥和前后两个安装版本。
...
- 不继续优先扩充依赖、自定义字段、资源排期等通用 PM 深水功能；
...
1. 用现有未完成 issue 收口 R0，逐项确认继续、改范围或关闭，避免重复登记；
2. 建立 10 个黄金任务的输入、期望产物、权限和验收夹具；
3. 为 Run/Artifact 领域模型登记一个总 epic，并把数据、API、SSE、UI 分片；
4. 从最高复用的文件操作和浏览器工具开始，再按黄金任务引入 DOCX/XLSX/PPTX/PDF；
5. 并行设计结构化权限 schema，但在门禁完成前不开放无人值守外部写；
6. 将能力发布和正式桌面更新作为独立链路演练，不能附带在目录 CRUD 中验收；
7. 每个阶段只在退出条件满足后进入下一阶段，不用“页面已出现”代替端到端完成。
