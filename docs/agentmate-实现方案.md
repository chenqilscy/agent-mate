...
> 从高保真原型（`docs/WorkBuddy/tencent-workbuddy-reference.html`，约 2200 行单文件）到可运行的真实产品。
> 原则：不硬编码、不模拟——所有流式输出来自真实 LLM，所有状态可持久化，所有轨迹是真实的 Agent 事件。
>
> **文档结构**：正文（第一～十二节）是结论与蓝图，是唯一权威；文末「附录 A」是各项选型的论证依据（为什么这么选、否掉了什么、何时重估）。评审先读正文，对某项选型有疑问再查附录对应决策。
>
> **实现进度（活文档，截至 2026-07-21）**：M0–M7、§11 项目工作台、Tauri 2 桌面壳、自动化/连接器/技能、独立 AgentMate Server、Console、多助理多渠道、知识库和模型管理均已有真实实现。当前重点不是继续补静态橱窗，而是 §12 的能力发布/兼容闭环与真实办公交付工具。**尚未完成**：M8 执行流实时围观、真实多 Agent 专家团、正式 Tauri 更新服务/代码签名、更多外部渠道和完整企业级灰度治理。
>
> **⚠️ 现状勘误（2026-07-14，WB-161）**：本文部分表述已被后续实现超越，以此处为准：
> 1. **独立 Server 已建成并运行**——下文若干历史段落仍保留最初“共享后端”排期语气；实际 epic WB-058~063 已落地独立同仓 `server/` 服务（FastAPI + SQLite，默认 `127.0.0.1:8100`，账号/组织/项目/成员·角色/邀请权威源 + 鉴权签发），本地 backend 作其客户端（local-first 执行 + 云端控制平面，见 `docs/agentmate-server-架构设计.md`）。
...
>
> **能力体系修订（2026-07-21，WB-235）**：目录 CRUD、Skill 本机安装/升级和 Tauri updater
> 脚手架不等于生产能力发布闭环。当前仍缺中心停用墓碑、Skill 指令/工具/权限原子快照、App
> 能力兼容门禁、在线目录失效通知和正式签名更新服务。权威目标见 §12；腾讯 WorkBuddy 参考资料
> 已集中到 [`docs/WorkBuddy/`](WorkBuddy/README.md)。
...
单机模式下无 token 时 auth 中间件注入固定本地用户——协作是“接上 Server”，不是改掉 local-first 执行。**当前实现**已包含独立同仓 `server/` 控制面；App 未配置 `AGENTMATE_SERVER_URL` 时保持纯本地，配置后由本地 backend 代理鉴权、目录和协作同步。企业 SSO、产物按需上云和多区域 SaaS 仍属后续。
...
| **M5 技能与连接器**（✅已落地） | MCP 客户端接入真实连接器（内置 本地便签/时间助手/工作区检索/Telegram/金山文档 + 第三方 GitHub 经 npx）、技能=可注入的指令+本机安装快照+**真实工具包**（`web_fetch` / `html_to_markdown` / `analyze_csv` / `create_local_skill`）、专家=预设人格；**Tauri 2 壳集成**（无边框窗口+PyInstaller sidecar，详见路线 A） | 已安装且启用的技能/连接器可被运行时真实解析；未知能力不伪装生效 |
| **M6 打磨**（✅主体已落地） | 深色主题持久化 ✅、⌘F 对话内搜索 ✅（CSS Custom Highlight API）、900px 响应式抽屉 ✅、安装包分发 ✅（MSI/NSIS）；余：更新端点 + 代码签名（需用户基建/证书） | 上述均在浏览器/桌面壳实测通过 |
| **M7 协作版**（✅ C1–C4 + 独立 Server 已落地） | 真账户鉴权、项目成员·角色·邀请、队友项目会话只读可见 + 动态署名、消息中心真事件；其后 WB-058~063 已把架构升级为独立 Server 控制面 + 本地执行端，未连接时纯本地回退 | 两账户协作门禁与 Server/App 同步均有定向验证；实时执行流围观仍归 M8 |
...
安全上有三条硬线：LLM 输出必须经 DOMPurify 才能进 `dangerouslySetInnerHTML`；文件工具严格锁在 workspace 沙箱并记录真实事件；API Key 只存后端 `.env` 或本机 DB，前端永不接触。`run_command` 目前虽固定 cwd、限时、剔除密钥并受黑名单审计，但它仍以后端权限执行，**不是真沙箱**，不得把它描述成工作区边界已经完备。性能上注意两点：长轨迹（几百个事件）用虚拟列表或分段折叠渲染；SSE 断线重连要带 `last_event_id` 续传。数据层从 M1 起按多用户预埋（UUID 主键、owner_id/project_id、角色枚举、auth 中间件桩），协作拓扑为 Local Agent + Server（详见附录 A.3）。工程习惯上，视觉原型保存在 `docs/WorkBuddy/tencent-workbuddy-reference.html`，官方资料和产品分析见 `docs/WorkBuddy/`；视觉验收可以对照原型，但功能验收必须以真实工具、持久化和端到端结果为准。
...
> **进展更新（2026-07-21）**：独立 Server/Console、目录下发、Skill 安装与手动版本升级均已落地；下一阶段不再继续堆静态能力卡，而优先完成 §12 的能力发布闭环和真实办公交付工具。M8 实时围观、更多外部渠道与真实多 Agent 专家团按依赖后置。
...
> 本节是 2026-07-21 起的产品与工程权威路线。腾讯 WorkBuddy 的官方事实与本项目对标分析见
> [`docs/WorkBuddy/`](WorkBuddy/README.md)；AgentMate 不照搬动态商品数量、价格或未验证能力。

### 12.1 产品原则：任务交付优先

AgentMate 的一级对象是**任务**，不是无限增长的聊天。任务绑定工作区、模式、模型和 loadout，
执行过程展示计划与真实工具事件，完成后交付可打开、可检查、可继续编辑的文件/页面/数据结果。

能力分层保持单一职责：

| 层级 | 职责 | 真实性门槛 |
|---|---|---|
| Tool | 最小可执行动作 | 有代码实现、结构化 schema、权限边界、事件与测试 |
| Skill | 指令 + 工具 + 附加资料/工作流 | 可安装、可启停、版本一致、工具真实解析 |
| Expert | 人设 + 领域方法论 + 可用能力 | persona 真注入；不可把展示卡当运行人格 |
| Expert Team | 团长拆解 + 多角色并行 + 审阅汇总 | 独立上下文、真实子任务、预算和可观察记录 |
| Connector | 外部系统身份、授权、数据和写操作通道 | 凭据本机保存、独立授权、读写范围明确 |
| Automation | 定时触发一个完整任务 | 绑定工作区/模型/Skill，限制无人值守风险并留审计 |
| Inspiration/Template | 发现案例并预填任务配置 | 只能引用真实可用的 Prompt/Skill/Expert |

### 12.2 当前内置能力边界

Console 可绑定给 Skill 的注册工具目前只有 4 个：`web_fetch`、`html_to_markdown`、
`analyze_csv`、`create_local_skill`。内置目录 Skill 目前 6 个：Web Access、MarkItDown、技能创建
指南、Word 文档生成、Excel 文件处理、股票综合分析器。

其中 Word 和股票 Skill 是纯指令技能：前者尚不能生成 `.docx`，后者没有权威实时行情工具。
在真实工具补齐前，文档/UI 必须按实际能力描述或限制输入来源，不得通过名称暗示已经完成文件或
市场数据交付。
...
| 更新对象 | 权威与分发 | 目标行为 |
|---|---|---|
| 目录展示/推荐 | Server catalog → App 镜像 | revision/ETag 检查；显式停用可压制 builtin |
| Skill 内容快照 | Server 定义 → 本机安装目录 | 指令、工具、文件、权限、版本、哈希原子升级 |
| 工具能力契约 | App 代码注册表 + Server 可选目录 | App 上报能力；Server 按最低版本/权限兼容发布 |
| App 运行时/UI/工具实现 | 签名 Tauri 安装包 | stable/beta、灰度、强制最低版本、失败回滚 |

Server 不得通过目录数据创造旧 App 没有的工具，也不得无签名地下发任意 Python/PowerShell
执行代码。纯文本且不扩大权限的内置 Skill 可自动升级；新增文件写入、网络或外部服务权限时必须
重新确认。App 离线时保留最后可用快照；“Server 明确撤回”与“Server 暂时不可达”必须区分。

### 12.4 建设优先级

1. **P0 发布闭环**：停用 tombstone、Skill 原子权限快照、App capability report、兼容门禁、
   目录刷新/失效通知、正式 Tauri 更新服务。
2. **P0 办公交付**：DOCX/XLSX/PPTX/PDF 创建编辑与渲染、浏览器自动化、文件搜索/补丁/整理；
   每类工具执行“生成 → 渲染 → 检查 → 修正”。
3. **P1 连接与多模态**：邮箱、日历、企业文档/IM、OCR、图片、音频、深度研究和自动化结果投递。
4. **P2 真实专家团**：团长调度、子任务 DAG、并行成员、审稿汇总、预算和失败降级。

具体 Server/Console/同步/桌面发布契约分别见对应专题文档。本节定义方向，不把尚未改动的运行时
标记为已完成；实施时按 issue 流程逐项登记和验收。
...
- 决策三 → M1 起数据层多用户预埋；M7 协作 C1–C4 与独立 Server 控制面均已落地；M8 实时执行流围观未做。
