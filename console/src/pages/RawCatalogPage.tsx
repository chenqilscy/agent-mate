...
export default function RawCatalogPage() {
  ...
  const columns: ProColumns<CatalogItem<CatalogData>>[] = [
    ...
    { title: "启用", dataIndex: "enabled", width: 80, fixed: "right", render: (_value, item) => <Switch aria-label={`${item.category} 目录项启用状态`} size="small" checked={item.enabled} onChange={async (enabled) => { await consoleApi.updateCatalogItem(item.id, { enabled }); await load(); }} /> },
    { title: "操作", valueType: "option", width: 150, fixed: "right", render: (_value, item) => <Space><Button type="link" size="small" icon={<EditOutlined />} onClick={() => open(item)}>编辑</Button><Popconfirm title="删除此目录项？" onConfirm={async () => { try { await consoleApi.deleteCatalogItem(item.id); message.success("已删除"); await load(); } catch (reason) { message.error(reason instanceof Error ? reason.message : "删除失败"); } }}><Button type="link" danger size="small" icon={<DeleteOutlined />}>删除</Button></Popconfirm></Space> },
  ];
  return (
