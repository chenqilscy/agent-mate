...
class WorkbenchStateAccuracyTests(unittest.TestCase):
    ...
    def test_home_uses_domain_specific_snapshots_and_neutral_visible_focus(self) -> None:
        ...
        focus_rule = css.split(".home-quick-start.is-keyboard-navigation .composer:focus-within", 1)[1].split("}", 1)[0]
        self.assertIn("var(--text-2)", focus_rule)
        self.assertNotIn("var(--brand)", focus_rule)
