...
class DesktopDualChannelTest(unittest.TestCase):
    ...
    def test_frontend_has_distinct_clients_and_no_legacy_server_proxy(self) -> None:
        ...
        automation_store = (repository / "src" / "stores" / "automationStore.ts").read_text(encoding="utf-8")
        ...
        self.assertIn("finalizeServerTurn", sse)
        self.assertIn("history.slice(-200)", sse)
        self.assertIn("automation_id: opts.automationId", sse)
        self.assertIn("streamChat", automation_store)
        self.assertNotIn("api.runAutomation", automation_store)
