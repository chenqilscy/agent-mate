...
c.check("B2 file in THIS project sandbox", "ISO-6001" in (read_ws(pid, "iso.txt") or ""))
...
# ── B4 plan-item write-back: agent transitions a work item, DB + SSE reflect ──
c.section("B4 计划项回写(set_work_item_status → SSE work_item + DB)")
wi = call("POST", "/work-items", tok, {"project_id": pid, "title": "验收:回写测试项", "status": "todo"})[1]
wid = wi["id"]
evs, sid = stream(tok, {"text": f"用 set_work_item_status 工具把 item_id={wid} 的计划项状态改为 完成。只做这一件事，完成后停止。", "project_id": pid},
                  stop_when=lambda e: e["event"] == "work_item", max_seconds=45)
time.sleep(0.4); stop_run(tok, sid)
wiev = events_of(evs, "work_item")
c.check("B4 work_item live SSE event emitted with status done",
        bool(wiev) and wiev[0]["data"].get("item", {}).get("status") == "done", str(wiev[:1]))
db_status = call("GET", f"/work-items?project={pid}", tok)[1]["items"]
this = next((x for x in db_status if x["id"] == wid), {})
c.check("B4 work item persisted as done in DB", this.get("status") == "done", str(this.get("status")))
...
stop_run(tok, sid)
c.check("B5 list_work_items step emitted", has_step(evs, "list_work_items"))
