...
BUILTIN_SKILLS: list[dict[str, Any]] = [
    ...
    {"slug": "skill-creator-guide", "name": "技能创建指南", "icon": "🧩", "category": "开发编程",
     "description": "通过对话梳理技能用途、触发场景与执行指令，并安装为本机技能。",
     "instructions": "帮助用户创建自定义技能：先澄清用途、触发场景、输入输出与约束，整理出稳定英文 slug、名称、描述和完整 Markdown 指令；信息足够后必须调用 create_local_skill 真正创建并安装，不要只给模板或假装已创建。",
     "tools": ["create_local_skill"]},
