...
WB-164 —— **Manager 权威 + 离线兜底**：接了 Manager（HUB_URL 设了）时，登录/注册以
Manager（Hub）账号库为准，App token 即 Hub token（校验后镜像进本地 users）。这样在 Manager
建的账号能登 App、在 App 注册的账号 Manager 也能看到——两端用户打通。Manager 不可达时：
登录回退本地 users（离线/单机仍可登），注册诚实报 503（不静默建分裂的本地账号，遵循 WB-158）。
未接 Manager（HUB_URL 空）→ 完全走本地路径，纯本地零变化（离线优先）。
"""
...
def _mirror_hub_account(token: str, acct: dict) -> dict:
    """把 Manager 校验过的账号镜像进本地 users + 缓存 Hub token（后续请求本地命中，不再打 Manager），
    并记住 Hub 身份供后台 outbox 以本人推送。返回 {token, user}——token 即 Hub token。"""
    aid = str(acct.get("id") or "")
    if not aid:
        raise HTTPException(502, "Manager 返回了异常账号")
    db.upsert_external_user(aid, str(acct.get("name", "")), str(acct.get("plan", "体验版")))
    db.cache_token(token, aid)
    db.set_hub_identity(aid, token)
...
def register(body: RegisterBody) -> dict:
    ...
    # 接了 Manager → 以 Manager 为权威源注册（WB-164）。拒绝(重名/非法)透传 4xx；不可达诚实 503。
    if hub_client.hub_enabled():
        status, data = hub_client.hub_login_ex(name, body.password, register=True)
        if status == "ok" and data:
            return _mirror_hub_account(data["token"], data.get("account") or {})
        if status == "rejected":
        ...
        raise HTTPException(503, "Manager 暂不可达，无法注册（请稍后重试）")
    # 未接 Manager → 纯本地注册（离线优先，零变化）。
...
def login(body: LoginBody) -> dict:
    name = (body.name or "").strip()
    # 接了 Manager → 以 Manager 为权威源验证（WB-164）。ok 用 Hub token 镜像；rejected(密码错/无此账号)
    # 直接 401（不回退，避免本地放行错误密码）；unreachable 才回退本地 users（离线/单机可登）。
    if hub_client.hub_enabled():
        status, data = hub_client.hub_login_ex(name, body.password, register=False)
        if status == "ok" and data:
            return _mirror_hub_account(data["token"], data.get("account") or {})
        if status == "rejected":
